...
def _read_encrypted_yaml_simple(path: Path) -> Dict[str, str]:
    if not path.exists():
        return {}
    key_b64 = (os.environ.get("DB_CRED_KEY") or "").strip()
    if not key_b64:
        log_event("DB_CRED_KEY missing; cannot decrypt db_cred.enc")
        return {}
    try:
        key = base64.urlsafe_b64decode(key_b64)
    except Exception:
        log_event("DB_CRED_KEY invalid; cannot decode base64")
        return {}
    blob = path.read_bytes()
    if len(blob) < 13:
        log_event("db_cred.enc is too short to contain nonce+ciphertext")
        return {}
    nonce, ciphertext = blob[:12], blob[12:]
    try:
        plaintext = AESGCM(key).decrypt(nonce, ciphertext, None)
    except Exception as e:
        log_event(f"db_cred.enc decrypt failed: {e}")
        return {}
    return _parse_yaml_simple(plaintext.decode("utf-8", errors="ignore"))


def _load_sqlserver_config() -> Dict[str, str]:
    # Look for db_cred.enc in repo root
    root = Path(__file__).resolve().parent
    enc_path = root / "db_cred.enc"
    cfg = _read_encrypted_yaml_simple(enc_path)
    if cfg:
        return cfg
    return {}
...
def _connect(cfg: Dict[str, str]):
    ...
    try:
        ...
        username = cfg.get("username", "")
        password = cfg.get("password", "")
        fallback_errors = [f"ODBC Driver 17 for SQL Server: {first_error}"]
        fallbacks = [
            (
        ...
        for driver_name, fallback_conn_str in fallbacks:
            try:
                return pyodbc.connect(fallback_conn_str, timeout=10)
            except Exception as fallback_error:
                fallback_errors.append(f"{driver_name}: {fallback_error}")
                continue
        log_event("SQL Server connection failed for all drivers: " + " | ".join(fallback_errors))
        raise first_error
...
def ensure_table(conn, table: str = DEFAULT_TABLE) -> None:
    ...
    END
    -- Purchase invoice processing is append-only: repeated uploads are kept as
    -- separate header rows to mirror the line-level processing history.
    IF EXISTS (
        SELECT 1
        FROM sys.indexes
        WHERE name = 'UX_invoice_headers_natural'
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM sys.indexes
        WHERE name = 'IX_invoice_headers_natural'
          AND object_id = OBJECT_ID('dbo.{table}')
    )
    BEGIN
        CREATE INDEX IX_invoice_headers_natural
...
def insert_customer_profile_sqlserver(
    ...
    if not cfg:
        log_event("SQL Server config missing (db_cred.enc not found or empty)")
        return False, "db_cred.enc not found or empty"
    try:
        conn = _connect(cfg)
    except Exception as e:
...
def insert_invoice_lines_sqlserver(
    ...
    def _pack_size_from_parts(cp: Any, us: Any) -> Optional[str]:
        try:
            if cp in (None, "") or us in (None, ""):
                return None
            cp_s = str(cp).strip()
            us_s = str(us).strip()
            if not cp_s or not us_s:
                return None
            return f"{cp_s}x{us_s}"
        except Exception:
            return None
...
def get_invoice_header_id(
    ...
    try:
        ...
        cur.execute(
            ...
            SELECT TOP 1 id
            FROM dbo.{table}
            WHERE ((? IS NOT NULL AND invoice_fingerprint=?) OR (? IS NULL AND
                   COALESCE(doc_type, '') = COALESCE(?, '')
              AND COALESCE(supplier_name, '') = COALESCE(?, '')
              AND COALESCE(doc_number, '') = COALESCE(?, '')
              AND COALESCE(doc_date, '') = COALESCE(?, '')))
            ORDER BY id DESC
            """,
            (
                header.get("invoice_fingerprint"), header.get("invoice_fingerprint"), header.get("invoice_fingerprint"),
                header.get("doc_type"),
        ...
        log_event(f"SQL header id fetch failed: {e}")
        return None
    finally:
        try:
            conn.close()
...
def get_invoice_line_id_range(
    ...
    try:
        ...
        cur.execute(
            f"""
            SELECT MIN(id) AS line_id_start, MAX(id) AS line_id_end
            FROM dbo.{table}
            WHERE ((? IS NOT NULL AND invoice_fingerprint=?) OR (? IS NULL AND
                   COALESCE(doc_type, '') = COALESCE(?, '')
              AND COALESCE(supplier_name, '') = COALESCE(?, '')
        ...
        if not row:
            return None, None
        start_id = int(row[0]) if row[0] is not None else None
        end_id = int(row[1]) if row[1] is not None else None
        return start_id, end_id
    except Exception as e:
        log_event(f"SQL line id range fetch failed: {e}")
        return None, None
    finally:
        try:
            conn.close()
...
def build_vendor_margin_change_report(
    vendor_no: str,
    doc_number: Optional[str] = None,
    doc_date: Optional[str] = None,
    supplier_name_hint: Optional[str] = None,
    lines_table: str = DEFAULT_LINES_TABLE,
    item_table: str = DEFAULT_ITEM_TABLE,
    tolerance: float = 0.0001,
) -> Tuple[bool, str, list[Dict[str, Any]]]:
    """
    Compare invoice unit_price (DocMate invoice lines) vs current item cost for a vendor.

    Matching strategy:
    - invoice code -> Item.Vendor_Item_No

    Returns:
    - ok flag
    - message
    - rows with change direction and recalculated margin
    """
    ...
    vno = (vendor_no or "").strip()
    if not vno:
        return False, "vendor_no is required", []
    ...
    try:
        ...
        # If doc_number/doc_date are not provided, pick latest document for vendor hint.
        # supplier_name_hint defaults to vendor_no so BOOKER can pick Booker invoices.
        sup_hint = (supplier_name_hint or vno).strip()
        like_hint = f"%{sup_hint.upper()}%"
        resolved_doc_number = (doc_number or "").strip()
        resolved_doc_date = (doc_date or "").strip()

        if not resolved_doc_number or not resolved_doc_date:
            cur.execute(
                f"""
                SELECT TOP 1 doc_number, doc_date, supplier_name
                FROM dbo.{lines_table}
                WHERE NULLIF(LTRIM(RTRIM(COALESCE(doc_number, ''))), '') IS NOT NULL
                  AND NULLIF(LTRIM(RTRIM(COALESCE(doc_date, ''))), '') IS NOT NULL
                  AND (
                        ? = ''
                        OR UPPER(COALESCE(supplier_name, '')) LIKE ?
                      )
                ORDER BY id DESC
                """,
                (sup_hint, like_hint),
            ...
            picked = cur.fetchone()
            if not picked:
                return False, "No invoice found to build report", []
            resolved_doc_number = str(picked[0] or "").strip()
            resolved_doc_date = str(picked[1] or "").strip()

        cur.execute(
            f"""
            ;WITH invoice_lines AS (
                SELECT
                    LTRIM(RTRIM(code)) AS invoice_code,
                    MAX(description) AS invoice_description,
                    AVG(CAST(unit_price AS float)) AS invoice_unit_price,
                    SUM(COALESCE(qty, 0.0)) AS invoice_qty
                FROM dbo.{lines_table}
                WHERE doc_number = ?
                  AND doc_date = ?
                  AND NULLIF(LTRIM(RTRIM(COALESCE(code, ''))), '') IS NOT NULL
            ...
            SELECT
                ...
                ? AS doc_date,
                IL.invoice_code,
                IL.invoice_description,
                IL.invoice_qty,
                IL.invoice_unit_price,
                I.Item_No AS matched_item_no,
                I.Vendor_Item_No AS matched_vendor_item_no,
                CAST(I.Price AS float) AS local_price,
                CAST(I.Cost AS float) AS local_cost,
                CAST(I.Margin AS float) AS local_margin_pct
            FROM invoice_lines IL
            OUTER APPLY (
                SELECT TOP 1 X.*
                FROM dbo.{item_table} X
                WHERE UPPER(LTRIM(RTRIM(COALESCE(X.Vendor_No, '')))) = UPPER(?)
                  AND LTRIM(RTRIM(COALESCE(X.Vendor_Item_No, ''))) = IL.invoice_code
                ORDER BY
                    X.Last_Modified DESC,
                    X.Item_No
            ) I
            ORDER BY IL.invoice_code
            """,
            (
                resolved_doc_number,
                resolved_doc_date,
        ...
        for row in db_rows:
            ...
            inv_price = _to_float(rec.get("invoice_unit_price"))
            local_cost = _to_float(rec.get("local_cost"))
            local_price = _to_float(rec.get("local_price"))
            local_margin = _to_float(rec.get("local_margin_pct"))

            delta_cost = None
            direction = "no_match"
            if inv_price is not None and local_cost is not None:
                delta_cost = round(inv_price - local_cost, 4)
                if abs(delta_cost) <= tolerance:
                    direction = "no_change"
                elif delta_cost > 0:
                    direction = "increase"
                else:
            ...
            margin_if_new_cost = None
            margin_delta = None
            if inv_price is not None and local_price is not None and local_price > 0:
                margin_if_new_cost = round(((local_price - inv_price) / local_price) * 100.0, 4)
                if local_margin is not None:
                    margin_delta = round(margin_if_new_cost - local_margin, 4)

            rec["delta_cost"] = delta_cost
            rec["change_direction"] = direction
            rec["margin_if_new_cost_pct"] = margin_if_new_cost
            rec["margin_delta_pct"] = margin_delta
            out.append(rec)

        msg = f"ok ({len(out)} rows, invoice={resolved_doc_number}/{resolved_doc_date}, vendor={vno})"
        return True, msg, out
    except Exception as e:
        return False, f"SQL margin report failed: {e}", []
    finally:
        try:
            conn.close()
        except Exception:
...
def list_recent_invoice_headers_sqlserver(
    vendor_no: Optional[str] = None,
    table: str = DEFAULT_TABLE,
    limit: int = 100,
) -> Tuple[bool, str, list[Dict[str, Any]]]:
    """
    Fetch recent invoice headers to help pre-fill report filters in the UI.
    """
    cfg = _load_sqlserver_config()
    if not cfg:
        return False, "db_cred.enc not found or empty", []
    ...
    try:
        ...
        return True, f"ok ({len(out)} headers)", out
    except Exception as e:
        return False, f"SQL header list failed: {e}", []
    finally:
        try:
            conn.close()
        except Exception:
...
def list_invoice_lines_sqlserver(
    doc_number: str,
    doc_date: str,
    vendor_no: Optional[str] = None,
    table: str = DEFAULT_LINES_TABLE,
    ...
    """
    Fetch uploaded invoice line details for checking in Reports UI.
    """
    cfg = _load_sqlserver_config()
    if not cfg:
        return False, "db_cred.enc not found or empty", []

    doc_no = (doc_number or "").strip()
    doc_dt = (doc_date or "").strip()
    if not doc_no or not doc_dt:
        return False, "doc_number and doc_date are required", []
    ...
    try:
        ...
        cur.execute(
            ...
            """,
            (doc_no, doc_dt, vno, like_hint),
        )
        cols = [c[0] for c in cur.description]
        out = [dict(zip(cols, row)) for row in cur.fetchall()]
        return True, f"ok ({len(out)} lines)", out
    except Exception as e:
        return False, f"SQL invoice lines fetch failed: {e}", []
    finally:
        try:
            conn.close()
        except Exception:
...
def insert_invoice_header_sqlserver(
    ...
    try:
        ...
        if fingerprint:
            cur.execute(f"SELECT TOP 1 1 FROM dbo.{table} WHERE invoice_fingerprint=?", (fingerprint,))
            if cur.fetchone():
                cur.execute(f"SELECT TOP 1 1 FROM dbo.{DEFAULT_LINES_TABLE} WHERE invoice_fingerprint=?", (fingerprint,))
                return (True, "duplicate") if cur.fetchone() else (True, "resume_lines")
        cur.execute(
            f"""INSERT INTO dbo.{table} (
                created_utc, filename, doc_type, supplier_name, customer_name, cms_customer_id,
                doc_number, doc_date, total_ex_vat, total_vat, total_inc_vat, currency,
                raw_header_json, invoice_fingerprint
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                row.get("created_utc"),
                row.get("filename"),
        ...
        conn.commit()
        log_event(f"SQL Server header insert ok into dbo.{table}")
        return True, "ok"
    except Exception as e:
...
def get_expenses_header_id(
    ...
    try:
        ...
        log_event(f"SQL expenses header id fetch failed: {e}")
        return None
    finally:
        try:
            conn.close()
